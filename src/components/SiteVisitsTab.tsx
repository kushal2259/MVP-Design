          ...
          <div key={v.id} style={{ border: '1px solid var(--line)', borderRadius: 10, backgroundColor: 'white', marginBottom: 14, overflow: 'hidden' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 14, padding: '14px 18px', flexWrap: 'wrap' }}>
              ...
              <div style={{ display: 'flex', gap: 6 }}>
                {v.status === 'scheduled' && <button onClick={() => sendTestReminder(v)} style={{ padding: '5px 10px', borderRadius: 5, border: `1px solid ${v.reminderEmail ? 'var(--blueprint)' : 'var(--line-strong)'}`, background: 'white', color: v.reminderEmail ? 'var(--blueprint)' : 'var(--steel)', fontSize: 11, cursor: 'pointer', opacity: v.reminderEmail ? 1 : 0.6 }} title={v.reminderEmail ? `Send a test reminder to ${v.reminderEmail}` : 'Add a Reminder Email to this visit first (click Report ✎ to edit)'}>✉ Test</button>}
